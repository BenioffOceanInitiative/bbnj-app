...
for (r in 2:4){
  ...
  d %>% 
    select(-hexres) %>% 
    pivot_wider(
      names_from  = sid, names_prefix = "s", 
      values_from = hexpct) %>% 
    write_csv(
      glue("{dir_gdata}/solution_hexids_res{r}_wide.csv"))
}
# read_csv(glue("{dir_gdata}/solution_hexids_res{r}_wide.csv")) %>% names()
...
dir_gdata  <- "/Users/bbest/My Drive/projects/bbnj-app/data"
dir_big    <- "/Users/bbest/big/bbnj-app_data"
dir_tiles  <- glue("{dir_big}/hex_tiles")
sh         <- "/bin/sh"
script_sh  <- glue("{dir_big}/hex_tiles_make.sh")
script_log <- glue("{dir_big}/hex_tiles_log.txt")
...
d_tilesets <- tribble(
  ~h, ~z_min, ~z_max,
   ...
   3,      4,      6,
   4,      7,     10)
w <- function(ln, append=T, ...){
  write(paste(ln, "\n"), file=script_sh, append=append, ...)
}
glue("
  #!{sh}
  set -x # echo commands
  ...
  exec >'{script_log}' 2>&1") %>% w(append=F)
opts='--force --no-tile-compression --coalesce-densest-as-needed --layer=pct_pu --description="percent in conservation solution of hexagonal planning units (~42x42km)"'
for (i in seq_along(d_tilesets)){ # i = 1
  with(
    d_tilesets[i,], {
    glue(
      'echo "h:{h}, z_min:{z_min}, z_max:{z_max}"
      # geojson to mbtiles
      echo `date`
      tippecanoe --output="{dir_big}/hex_res{h}.mbtiles" \\
        {opts} --minimum-zoom={z_min} --maximum-zoom={z_max} \\
        "{dir_gdata}/hex_res{h}.geojson"
      # add percent solution columns `s#` to mbtiles
      echo `date`
      tile-join -f -o "{dir_big}/hex_res{h}_sol.mbtiles" \\
        -x hexid -c "{dir_gdata}/solution_hexids_res{h}_wide.csv" \\
        "{dir_big}/hex_res{h}.mbtiles"') %>% w()
    })
}
glue("
  ...
  tile-join --no-tile-compression --force -e '{dir_tiles}_no-tile-compression' \\
    '{dir_big}/hex_res2_sol.mbtiles' \\
    '{dir_big}/hex_res3_sol.mbtiles' \\
    '{dir_big}/hex_res4_sol.mbtiles'

  # join resolutions into directory of tiles, with tile compression
  echo `date`
  tile-join --force -e '{dir_tiles}' \\
    '{dir_big}/hex_res2_sol.mbtiles' \\
    '{dir_big}/hex_res3_sol.mbtiles' \\
    '{dir_big}/hex_res4_sol.mbtiles'
  ...
  echo `date`
  tar -jcf '{dir_tiles}.tar.bz2'                     '{dir_tiles}'
  echo `date`
  tar -jcf '{dir_tiles}_no-tile-compression.tar.bz2' '{dir_tiles}_no-tile-compression'
  ...
  echo `date`
  mv '{dir_tiles}.tar.bz2'                     '{dir_gdata}/{basename(dir_tiles)}.tar.bz2'
  echo `date`
  mv '{dir_tiles}_no-tile-compression.tar.bz2' '{dir_gdata}/{basename(dir_tiles)}_no-tile-compression.tar.bz2'
  
  # finished
  echo `date`") %>% w()

system(glue("{sh} {script_sh}"))
# TODO: move script_sh, script_log to dir_gdata
fs::file_copy(script_sh, glue("{dir_gdata}/{basename(script_sh)}"))
```

```
2.1G	/Users/bbest/big/bbnj-app_data/hex_tiles
573M	/Users/bbest/big/bbnj-app_data/hex_tiles.tar.bz2
 66M	/Users/bbest/big/bbnj-app_data/hex_tiles_log.txt
4.0K	/Users/bbest/big/bbnj-app_data/hex_tiles_make.sh
4.5G	/Users/bbest/big/bbnj-app_data/hex_tiles_no-tile-compression
193M	/Users/bbest/big/bbnj-app_data/hex_tiles_no-tile-compression.tar.bz2
```