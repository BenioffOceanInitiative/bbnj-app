...
FUNCTION tilebbnj(
    ...
    in_area float8, in_benthic float8, in_fishing float8, in_scapes float8, in_spp float8, in_vgpm float8,
...
AS $$
    ...
    FROM sol_hex2 AS s
    INNER JOIN hex_res2 h
    	ON s.hexid = h.hexid
    INNER JOIN sol_params p 
    	ON s.sid = p.sid
    WHERE 
    	p.area    = in_area AND
    	p.benthic = in_benthic AND 
    	p.fishing = in_fishing AND 
    	p.scapes  = in_scapes AND 
    	p.spp     = in_spp AND 
    	p.vgpm    = in_vgpm
    LOOP
       IF maxbounds ~ geom AND bounds && geom THEN
